...
Date: 2026-08-31
Status: public `main` baseline and immutable Release controls established; Monday-Wednesday-Friday candidate schedule gated by explicit repository variable
...
- Scheduled candidate creation uses Beijing Monday-Wednesday-Friday `07:30` and `16:30` only, with no Chinese holiday or adjusted-workday calendar. It remains fail-closed unless the repository owner sets `STEPHEN_DAILY_SCHEDULE_ENABLED` to `1` after the schedule PR and exact-main CI pass; that value enables only the existing candidate Draft-PR job and cannot approve, publish, Release, or deploy content.
- A Draft PR in a public repository is publicly visible. `not_published` means “not published on the website,” not “private.”
- AI may draft candidate copy but cannot approve content, change the review state, or publish an item. The workflow contract rejects automated `gh pr ready`, `gh pr merge`, and `gh pr review` commands.
...
{
  "status": "pass",
  "branchName": "codex/stephen-mwf-schedule",
  "scannedFiles": 106,
  "scannedBytes": 1135635,
  "workflowFiles": 4,
  "findings": []
}
```

## Human checks required for repository governance and schedule activation
...
6. Set `STEPHEN_DAILY_SCHEDULE_ENABLED` to string `1` only after the approved Monday-Wednesday-Friday schedule PR is merged and every workflow for the exact new `main` SHA succeeds; do not dispatch a real candidate run for activation testing.
7. Keep the merged SAAS-608 baseline and immutable Release controls intact; the candidate schedule must not add Release, deployment, Environment, server, DNS, Nginx, SSH, or traffic operations.

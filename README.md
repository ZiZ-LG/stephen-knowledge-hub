...
- 批准必须绑定当前 Draft PR 的完整 40 位 SHA；PR 漂移后旧批准失效。
- 合并后的 GitHub Release 指向人工批准封印 SHA，并在仓库启用 Immutable Releases 后锁定 tag 与资产。
- 每日候选 Draft PR 即使尚未进入网站，在 public 仓库中也对公众可见。
- 定时候选工作流默认关闭；只有仓库变量 `STEPHEN_DAILY_SCHEDULE_ENABLED=1` 时才执行定时任务。
- GitHub Release 是本公开闭环的终点，不会自动更新线上站点；操作说明见 [审核后发布闭环运行手册](docs/reviewed-release-runbook.md)。
